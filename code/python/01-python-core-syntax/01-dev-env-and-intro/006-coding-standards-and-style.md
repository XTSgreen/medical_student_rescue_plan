...
命名是编程中最难的事情之一。一个准确的名字能让代码自解释，一个含糊的名字会让维护者反复猜测。PEP 8 对不同类型的对象给出了不同的命名约定，掌握这些约定后，你看到名字就能判断它是变量、函数、类还是常量。
...
第一段注释完全是在翻译代码，任何人看到 `score = 72` 都知道是把分数设为 72，注释没有提供任何额外信息。第二段注释解释了为什么 52 这个看起来异常的值是正常的，这种背景信息在团队协作中很珍贵。
...
空格在代码中起到视觉分隔的作用，恰当的空格能让运算符和参数清晰可读。PEP 8 对空格的使用有详细规定，核心原则是在必要处加空格提升可读性，在不必要处删除空格避免冗余。
...
`from module import *` 是一种通配导入，它会把模块中所有公开名字一股脑倒入当前命名空间。这种写法看起来省事，是 Python 中容易出问题的导入方式，PEP 8 明确建议避免使用。
