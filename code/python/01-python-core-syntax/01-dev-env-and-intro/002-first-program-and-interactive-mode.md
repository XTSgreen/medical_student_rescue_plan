...
::: note REPL 的适用场景
REPL 适合验证一个函数的用法、测试一段逻辑、查看某个模块的属性。当代码需要反复运行或保存时，应该写成脚本文件。可以把 REPL 想象成临床的快速床旁检测——方便快捷但不能替代正式检验报告。
:::
...
Python 脚本文件以 `.py` 为扩展名。文件命名应遵循以下规则：只能使用字母、数字和下划线，不能以数字开头，区分大小写。按照 PEP 8 规范，文件名用小写字母加下划线分隔，例如 `patient_analysis.py`、`data_clean.py`。
...
可以把它理解成临床上的"留床观察"：暂时没有处理措施，但位置先占住，后续再补充。
