...
列表和元组是 Python 中最常用的两种序列类型。它们虽然一个可变一个不可变，但在索引、切片、拼接、比较等基础操作上完全一致。本节把这些共性操作集中讲解，帮助你一次掌握两套类型的通用用法，避免重复学习。理解这些操作后，你会发现列表和元组在读取层面几乎没有差异，区别仅在于能否原地修改。本节示例会同时用列表和元组演示，让你直观感受它们的操作一致性。
