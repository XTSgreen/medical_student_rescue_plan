...
元组是 Python 中另一种有序序列类型，外形与列表相似，但创建后不可修改。如果列表像可随时调整的清单，元组更像已封存的归档记录，一旦确立就不能再改动。本节将从字面量写法讲起，覆盖元组的索引、拆包、与列表的相互转换、不可变性细节以及命名元组，帮助你理解何时该用元组代替列表。
...
掌握元组后，你已经理解了 Python 中两种最基本的序列类型。下一节我们将进入字符串与字典等更复杂的数据类型，进一步扩展处理实际数据的能力。
