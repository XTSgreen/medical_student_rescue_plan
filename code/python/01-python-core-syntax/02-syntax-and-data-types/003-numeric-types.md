...
数字是编程中最基础的数据形式。在医学场景中，年龄、血压值、血糖浓度、心率、影像像素值、基因表达量等都需要用数字表示。Python 提供了四种内置数字类型，分别是整数（int）、浮点数（float）、复数（complex）和布尔类型（bool）。本节将依次介绍每种类型的字面量写法、取值范围、内部表示，以及类型转换、格式化输出和常用数值函数。掌握这些内容后，你就能在代码中准确表达各种数值，并避免因类型不当引发的精度误差或逻辑错误。
...
最常见的写法是**十进制**，也就是日常使用的 0 到 9 的数字组合，例如 `42`、`-7`、`0`。这是默认的进制，无需任何前缀。在记录病人年龄、病房床位号、样本编号这类日常数值时，都用十进制。

**二进制**字面量以 `0b` 或 `0B` 开头，后跟 0 和 1 的组合。例如 `0b1010` 表示十进制的 10。二进制在位运算、底层硬件控制、图像像素掩码等场景中使用。医学影像处理中，掩膜（mask）数据常以二进制位的方式存储每个像素是否属于感兴趣区域，这时理解二进制就很有用。

**八进制**字面量以 `0o` 或 `0O` 开头，后跟 0 到 7 的数字。例如 `0o17` 表示十进制的 15。八进制在 Unix 文件权限表示中常见，例如文件权限 `0o755` 表示拥有者可读写执行、其他用户可读可执行。日常医学数据处理中较少直接使用八进制。

**十六进制**字面量以 `0x` 或 `0X` 开头，后跟 0 到 9 和 A 到 F（大小写均可）。例如 `0xFF` 表示十进制的 255，`0x1A` 表示十进制的 26。十六进制在表示颜色值、内存地址、哈希值时非常常见。比如 DICOM 影像中像素值的某些范围、加密后的医疗数据指纹，常以十六进制字符串形式呈现。
...
在医学计算中，组合概率、排列组合数、大整数哈希等都可能产生非常大的数值。Python 自动扩展整数位数，让你无需关心溢出问题。在 Python 2 中曾有 `int`（固定宽度）和 `long`（任意宽度）两种类型，Python 3 统一合并为 `int`，使用上更加简单。
...
第一种是**十进制小数形式**，包含整数部分、小数点和小数部分。例如 `3.14`、`-0.5`、`0.0`。整数部分或小数部分可以为零，但小数点必须保留：`0.5` 可以简写为 `.5`，`5.0` 可以简写为 `5.`。在医学数据中，体温 `36.7`、血糖 `5.6`、收缩压 `120.0` 等都用这种形式。

第二种是**科学计数法**，用 `e` 或 `E` 表示 10 的幂。例如 `1.2e-3` 表示 $1.2 \times 10^{-3}$，即 `0.0012`；`6.022e23` 表示阿伏加德罗常数。科学计数法在表示极大或极小的数值时非常方便，比如基因表达量中的 p 值、药物浓度、显微镜下的细胞尺寸。
...
::: warning 浮点数不能直接用 == 比较
由于精度问题，**永远不要用 `==` 直接比较两个浮点数是否相等**。正确做法是检查两者之差的绝对值是否小于某个容差（epsilon）。在医学统计中，p 值、相关系数等浮点结果的比较尤其要注意这一点。
...
涉及金额、药剂剂量等对精度要求严格的场景，应使用 `decimal.Decimal` 类型而非内置 `float`，它能精确表示十进制小数。后续章节会详细介绍。
...
复数在信号处理、量子物理、电气工程等领域有广泛应用。医学影像中的 MRI（磁共振成像）信号采集时获得的是复数数据，包含幅度和相位信息；心电图（ECG）信号的频域分析也涉及复数运算。日常医学数据处理中复数较少直接使用，但了解其表示方式有助于阅读相关算法代码。
...
布尔类型（bool）只有两个值：`True` 和 `False`，注意首字母大写。布尔值用于表示逻辑上的真与假，在条件判断、循环控制中大量使用。医学场景中，是否吸烟、是否患有高血压、是否完成随访等二元状态都可以用布尔值表示。
...
print(is_smoker, has_hypertension)  # True False
...
# 实际用途：统计 True 的个数
patients = [True, False, True, True, False]  # 是否吸烟
smoker_count = sum(patients)
print(smoker_count)   # 3
```

这种设计在统计计数场景中非常方便。例如有一组病人的吸烟状态列表，用 `sum()` 直接累加就能得到吸烟者人数。`issubclass(bool, int)` 可以验证这一继承关系。
...
这一功能在解析医学数据时有用。例如某些医学编码系统使用十六进制存储标识符，需要转成十进制参与计算时，`int(code, 16)` 即可。
...
::: warning NaN 检测陷阱
由于 `NaN != NaN` 成立，**不能用 `==` 判断一个值是否是 NaN**。必须使用 `math.isnan()` 函数。在医学数据清洗中，缺失值常被表示为 NaN（Pandas 中尤为常见），正确检测 NaN 是数据预处理的关键步骤。
...
医学数据展示中，这些格式化技巧很常用。血压值保留整数 `f"{sbp:.0f}/{dbp:.0f}"`、p 值显示三位有效数字 `f"{p:.3e}"`、人群规模加千位分隔符 `f"{pop:,}"`，都是实际场景。
...
医学统计中处理大量数据时，了解这一规则能避免困惑。如果你需要传统的四舍五入（.5 总是向上），可以使用 `decimal.Decimal` 配合 `ROUND_HALF_UP` 模式，或者在数值上加一个极小值（如 `math.floor(x + 0.5)`，但要注意浮点精度问题）。
:::
