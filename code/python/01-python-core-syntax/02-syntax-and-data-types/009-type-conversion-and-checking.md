...
Python 是动态类型语言，变量在运行时才确定类型。这种灵活性带来便利，也意味着数据在不同类型之间流动时需要可靠的转换与判断机制。本节覆盖显式类型转换函数、字符串与对象表示、类型判断的三种工具，以及 Python 独特的鸭子类型思想。掌握这些内容后，你就能在处理临床数据、解析用户输入、设计函数接口时，对类型流动有清晰把握。

## 2.9.1 显式类型转换函数

显式类型转换也叫**强制类型转换**，是用内置构造函数把一个类型的值转换成另一个类型。这与隐式转换不同：隐式转换由解释器自动完成（如 `1 + 2.0` 中整数 1 被自动转成浮点数），显式转换则需要程序员在代码中写明。在临床场景里，这好比把一份纸质病历中的"年龄"字段从字符串录入为数据库中的整数字段，必须显式指定目标类型。
...
`bool("0")` 的结果是 `True`，这是常见的坑：字符串 `"0"` 不是空字符串，所以为真。在医学数据处理中，读取 CSV 或用户输入得到的都是字符串，需要先把 `"0"` 转成数字再做布尔判断。
...
class Patient:
    def __init__(self, name, age):
        ...
        self.age = age
    def __str__(self):
        return f"患者({self.name}, {self.age}岁)"

p = Patient("张三", 45)
print(str(p))         # 患者(张三, 45岁)
```

### 容器类型之间的转换

`list()`、`tuple()`、`set()`、`frozenset()`、`dict()` 用于在不同容器之间转换。这些转换在医学数据处理中极为常用，比如把一组患者的重复 ID 去重、把键值对列表转成字典。
...
# 列表转集合：自动去重，但丢失顺序
ids = ["P001", "P002", "P001", "P003"]
print(set(ids))       # {'P001', 'P003', 'P002'}，顺序不保证
...
# dict() 从键值对序列构造字典
pairs = [("name", "李四"), ("age", 30), ("dept", "心内科")]
print(dict(pairs))    # {'name': '李四', 'age': 30, 'dept': '心内科'}
...
这好比病历里的两类记录：`str()` 是给患者看的病情说明，通俗易懂；`repr()` 是医生之间的专业记录，需要精确到可以复现。
...
class LabResult:
    ...
    def __repr__(self):
        # 面向开发者：精确可重建
        return f"LabResult(item={self.item!r}, value={self.value!r}, unit={self.unit!r})"

r = LabResult("血糖", 5.6, "mmol/L")
print(str(r))   # 血糖: 5.6mmol/L
print(repr(r))  # LabResult(item='血糖', value=5.6, unit='mmol/L')
```
...
在医学数据处理场景中，绝不要用 `eval()` 处理来自 CSV、数据库或用户表单的字符串。如果需要解析数值，用 `int()` 或 `float()`；如果需要解析结构化数据，用 `json.loads()`。`eval()` 仅在受控环境中（如自己写的测试代码、可信的字面量解析）才有用武之地，日常数据处理应避开它。

## 2.9.4 类型判断：type()

`type()` 返回一个对象的类型对象。这是最直接的类型查询方式，类似临床中通过实验室检查直接确认某个指标的类型。
...
Python 内置的迭代器协议就是鸭子类型的典型应用。任何实现了 `__iter__` 和 `__next__` 方法的对象都可以被 `for` 循环遍历，无论它是列表、字符串、文件对象，还是自定义的实验数据流。
...
records = PatientRecords(["病历A", "病历B", "病历C"])
# for 循环不关心 records 的类型，只要有 __iter__ 就能遍历
for r in records:
    print(r)
```

鸭子类型让 Python 的函数接口极其灵活，也带来一些代价：类型错误要到运行时才暴露，IDE 也难以提供精确的代码补全。对于大型项目，可以配合 `typing` 模块的类型注解和 mypy 静态检查工具，在保持灵活性的同时提前发现类型问题。对于医学数据处理脚本和原型开发，鸭子类型的简洁与自由往往正是所需的。
