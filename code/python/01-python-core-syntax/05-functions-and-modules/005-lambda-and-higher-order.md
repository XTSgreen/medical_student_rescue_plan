...
高阶函数是指接受函数作为参数，或返回函数作为结果的函数。上一章已经见过把函数作为参数传递的例子。高阶函数的核心思想是把操作抽象出来作为参数传递，让同一个高阶函数能配合不同的操作完成不同任务。Python 内置了几个常用的高阶函数，如 `map`、`filter`，`sorted`、`min`、`max` 也接受函数作为 `key` 参数，也是高阶函数。
