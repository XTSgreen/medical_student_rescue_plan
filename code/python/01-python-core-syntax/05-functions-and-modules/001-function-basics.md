...
Python 中函数是一等对象，与整数、字符串一样，可以被赋值给变量、作为参数传递、作为返回值返回。函数名本质上是一个变量，指向内存中的函数对象。理解这一点对掌握后续的高阶函数、装饰器等概念至关重要。
...
`make_greeting` 内部定义了 `greet` 函数，并把 `greet` 作为返回值。外部拿到的 `morning_greet` 实际上是内层函数对象，调用它时能用到外层函数的 `prefix` 变量，这就是闭包的雏形。
