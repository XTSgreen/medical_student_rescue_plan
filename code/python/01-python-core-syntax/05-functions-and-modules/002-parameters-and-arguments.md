...
位置参数是基本的参数形式。调用函数时，实参按顺序对应到形参，第一个实参给第一个形参，第二个实参给第二个形参，依此类推。参数的数量必须匹配，多了或少了都会报 `TypeError`。
...
两次调用都用了关键字参数，第二次故意把 `priority` 写在 `title` 前面，依然能正确匹配。关键字参数让调用意图清晰，不必回去看函数定义就能知道每个值的含义。位置参数和关键字参数可以混用，但位置参数必须写在关键字参数前面。
...
第二次调用时没有传 `tasks`，使用的是默认值，而这个默认值是第一次调用后被修改过的那个列表。`tasks=[]` 在定义时创建了一个空列表对象，后续所有省略 `tasks` 的调用都共享这一个对象。这几乎总是与预期不符。
...
把位置专属参数、普通参数、`*args`、关键字专属参数、`**kwargs` 全部组合起来，就得到完整的函数签名形式。
