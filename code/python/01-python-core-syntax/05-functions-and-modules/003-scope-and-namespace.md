...
与作用域紧密相关的概念是命名空间。命名空间是名字到对象的映射，可以理解为一个字典，键是变量名，值是变量指向的对象。每个作用域都有自己的命名空间，作用域查找变量本质上就是按规则查找对应的命名空间。
...
`locals()` 返回的字典包含了当前函数所有的局部变量和形参。在调试时用它查看当前作用域内的所有变量很方便。需要强调的是，不要依赖 `locals()` 修改局部变量，Python 不保证这种修改会生效。
