...
任务管理器的入口文件 `main.py` 里经常看到这样一段代码：`if __name__ == "__main__":`，下面放着启动程序的逻辑。这行代码的作用是什么？为什么要有这个判断？答案藏在模块的特殊属性 `__name__` 中。每个模块都自带一批以双下划线开头和结尾的特殊属性，它们记录了模块的名称、文件路径、文档等信息。本节讲解这些模块级特殊属性，重点是 `__name__` 和 `if __name__ == "__main__":` 这一最常用的模式。
