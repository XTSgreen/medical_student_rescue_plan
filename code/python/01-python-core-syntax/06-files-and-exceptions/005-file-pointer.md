...
`whence=0` 表示从文件开头计算偏移，`offset` 必须是非负整数。这是默认值，也是常用的参照点。`seek(0)` 表示回到文件开头。
...
文本模式下 `seek()` 的非零绝对偏移只有两种来源是安全的：一是 `tell()` 返回的值，二是 0。可以 `tell()` 记录位置后 `seek()` 回去，也可以 `seek(0)` 回到开头，其他偏移量在多字节编码下可能出错。
...
`os.fsync(fd)` 接受文件描述符，确保数据真正写入物理磁盘，而不仅是操作系统的缓冲区。这一操作性能开销大，只在数据完整性要求极高的场景使用，如数据库事务日志。
