...
**集中趋势**用均值、中位数与众数描述数据的代表值。均值利用了每一个观测值的信息，对正态分布数据最具代表性，但容易被极端值拉偏；中位数只取中间位置，对极端值不敏感，更适合偏态数据；众数适用于分类变量。**离散程度**用方差、标准差、四分位距与变异系数描述数据的散布范围。标准差与原数据同单位，常用；四分位距 IQR 等于第 75 百分位数减第 25 百分位数，对极端值稳健；变异系数是标准差除以均值，无量纲，便于比较不同量纲的变量。
...
分位数与百分位数在医学参考值制定中重要。儿童生长发育标准常用第 3、10、25、50、75、90、97 百分位数刻画，临床检验的正常范围常取第 2.5 与 97.5 百分位数。R 的 `quantile()` 函数可以计算任意百分位数，`fivenum()` 给出五数概括，`IQR()` 给出四分位距。当需要把不同量纲的变量放到同一尺度比较时，`scale()` 函数执行标准化，把数据转换为均值为 0、标准差为 1 的 z 分数，这是回归分析与聚类分析前常见的预处理步骤。
...
下表把统计中常用的几类分布与它们的 R 函数族列出，方便查阅。
...
正态分布、t 分布、卡方分布与 F 分布之间有清晰的逻辑关系，理解这层关系就理解了大半个推断统计。**正态分布**是连续变量常用的理论模型，许多生理指标近似服从正态分布。**t 分布**用于总体方差未知时对均值的推断，它的形状比正态分布略胖，自由度越大越接近正态；这意味着小样本下用 t 而非 z 是更诚实的做法。**卡方分布**是若干标准正态变量平方和的分布，方差估计与列联表检验都从它导出。**F 分布**则是两个独立卡方变量除以各自自由度之比，方差分析建立在 F 统计量之上。这四者的关系可总结为一句话：从正态出发，平方得卡方，相除得 F，配上未知方差替换就得到 t。
...
::: note
中心极限定理保证了样本均值的分布近似正态，但前提是总体方差有限。重尾分布如柯西分布不满足这一条件，样本均值不会收敛到正态。在生信分析中，基因表达量的对数变换常被使用，部分原因就是为了把重尾的原始分布压缩到方差有限的形态，让中心极限定理适用。
:::
...
`aov()` 是 R 中拟合方差分析的标准函数，配合 `summary()` 查看 F 检验结果，配合 `TukeyHSD()` 做常用的 Tukey 事后比较。Tukey HSD 控制所有两两比较的族错误率，是各组样本量相等时的首选方法。
...
设某地区成年男性身高服从正态分布,均值为 170 cm,标准差为 6 cm。用 R 计算:身高低于 160 cm 的概率;身高 95% 参考范围的上界;从该分布随机抽取 1000 个观测值并计算其均值。
...
`pnorm` 计算累积概率,`qnorm` 计算分位数,`rnorm` 生成随机数。三者构成正态分布的 d/p/q/r 函数族。`qnorm(0.975)` 给出双侧 95% 区间的上界,对应单侧 97.5% 分位数。
:::

### 第2题 t 检验选择

给定两组独立样本 `drug <- c(12, 15, 11, 14, 13, 16, 10, 14)` 与 `placebo <- c(8, 9, 7, 10, 8, 11, 9, 10)`,用 R 做独立样本 t 检验,并说明默认使用的是 Welch 校正还是 Student t 检验。

::: details 参考答案

```r
drug <- c(12, 15, 11, 14, 13, 16, 10, 14)
placebo <- c(8, 9, 7, 10, 8, 11, 9, 10)

# 默认 var.equal = FALSE,即 Welch 校正
t.test(drug, placebo)

# 若要 Student t 检验,需显式指定
t.test(drug, placebo, var.equal = TRUE)
```

`t.test()` 默认 `var.equal = FALSE`,使用 Welch 校正。Welch 校正不假设两组方差相等,对正态性假设更稳健,即使在方差齐性满足时也几乎不损失功效,因此被推荐为默认选择。若要使用合并方差的 Student t 检验,需显式设 `var.equal = TRUE`。
:::

### 第3题 卡方检验与 Fisher 检验选择

给定列联表 `mat <- matrix(c(3, 1, 2, 4), nrow = 2)`,判断应该用卡方检验还是 Fisher 精确检验,并写出对应代码。

::: details 参考答案

```r
mat <- matrix(c(3, 1, 2, 4), nrow = 2)
...
卡方检验要求每个单元格的期望频数大于 5。该列联表总样本量仅 10,期望频数普遍小于 5,卡方近似不可靠。Fisher 精确检验基于超几何分布直接计算概率,对小样本与稀疏数据更准确。判断依据是查看 `chisq.test(mat)$expected`,若多数单元格期望频数小于 5,应改用 Fisher 检验。
:::
...
原因:`mean(x)` 默认不忽略缺失值,若 `x` 中含有 `NA`,返回值就是 `NA`。这是 R 统计函数的保守策略,提醒用户数据存在缺失。

解决:加 `na.rm = TRUE` 参数,即 `mean(x, na.rm = TRUE)`。`sd()`、`median()`、`var()` 等函数同理。在数据清洗阶段先用 `sum(is.na(x))` 检查缺失情况,再决定是删除还是填充。

**错误 2 · `chisq.test` 给出警告 `Chi-squared approximation may be incorrect`**

原因:列联表中某些单元格的期望频数小于 5,卡方统计量的渐近分布不再可靠。这是小样本或稀疏列联表的常见问题。

解决:改用 `fisher.test()` 做 Fisher 精确检验。它基于超几何分布直接计算概率,不依赖大样本近似。也可以用 `chisq.test()` 的模拟参数 `simulate.p.value = TRUE`,但 Fisher 检验在 2×2 表中更标准。

**错误 3 · `t.test` 与 `var.equal` 默认值混淆**

原因:许多教材默认演示 Student t 检验(合并方差),但 R 的 `t.test()` 默认 `var.equal = FALSE`,使用 Welch 校正。两者在方差不等时结果差异显著。

解决:明确研究假设与数据特征。若两组方差齐性满足(可用 `var.test()` 检验),可设 `var.equal = TRUE` 用 Student t 检验;否则保留默认的 Welch 校正。报告结果时注明使用了哪种检验。

**错误 4 · 把 p 值当作效应大小**

原因:p 值衡量数据与原假设的相容程度,受样本量影响。大样本下即使毫无实际意义的微小差异也能达到统计显著,此时 p 值小并不代表效应大。

解决:报告结果时同时给出效应量与置信区间。t 检验用 Cohen's d,ANOVA 用 $\eta^2$,相关系数本身就是效应量。效应量独立于样本量,能反映差异的实际重要性。
