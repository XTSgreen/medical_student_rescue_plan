...
右下是**多用途面板**，包含 Files（文件管理）、Plots（图形预览）、Packages（包管理）、Help（帮助文档）、Viewer（HTML 内容预览）。其中 Help 标签值得留意，在控制台输入 `?mean` 就会在这里显示 `mean` 函数的文档。
...
这里的运算过程是 `c(1,2,3) + c(10,20,10)`，短向量 `c(10,20)` 循环补齐成 `c(10,20,10)`，然后逐元素相加。循环补齐在向量化运算中常用，比如给数据框的每一列乘以不同的权重。
...
这并不意味着 `for` 循环在 R 中不能用。当循环次数少、或者每一步有复杂的副作用时，`for` 循环仍然清晰可读。但处理大量数据时，优先寻找向量化的写法。后续章节会介绍 `apply` 家族、`purrr::map` 等函数式工具，它们是向量化思维的延伸。
...
R 的运算符是函数。`+` 是函数 `"+"()` 的语法糖，`^` 是 `"^"()`。这意味着它们自动获得向量化行为，对向量的每个元素独立应用。统计函数同样如此：
...
向量是 R 的基本数据结构。前面已经用过多次，这里系统梳理它的创建、索引与命名。
...
矩阵是二维同质数据结构，数组是更高维的推广。它们是向量加上维度属性。
...
数据框（data frame）是 R 中常用的数据结构，相当于一张 Excel 表格：每列是一个变量（可以是不同类型），每行是一个观测。它是**等长向量的列表**，技术上 `data.frame` 继承自 `list`，但约束每列长度相同。
