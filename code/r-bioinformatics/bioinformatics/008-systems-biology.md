...
某研究者运行 WGCNA 时将软阈值幂（power）设为 1，理由是该值下纳入模块的基因数最多。分析 power = 1 的问题，并说明软阈值应如何选择。

::: details 参考答案
power = 1 等价于不应用软阈值，直接使用原始相关矩阵作为邻接矩阵。此时所有基因之间都有非零连接（只要相关系数不为零），网络过于密集，模块边界模糊，弱相关和强相关被同等对待，无法识别真正的共表达模块。WGCNA 的核心思想是用软阈值抑制弱相关，使网络接近无标度拓扑，强相关的基因聚成紧密模块，弱相关的基因连接被大幅压缩。软阈值选择通过 `pickSoftThreshold` 函数完成：对一系列 power 值（通常 1-20）计算网络的 scale-free topology fit index（R²）和 mean connectivity，选择 R² 首次超过 0.8（或 0.9）的 power 值。同时检查 mean connectivity 在该 power 下已降至较低水平。人类 RNA-seq 数据通常 power 在 6-12 之间。若所有 power 下 R² 都不达标，提示数据可能不适合 WGCNA，或需要先过滤低表达和低变异基因。
:::

### 第2题 参数分析

某研究者对两个条件（对照和处理）的单细胞数据分别运行 CellChat，发现处理组的总通信强度（total interaction strength）显著高于对照组，据此得出处理增强细胞间通讯的结论。研究者未按细胞群体大小归一化。说明该结论可能存在的偏差，并给出正确的比较方法。

::: details 参考答案
CellChat 的通信概率计算综合配体表达、受体表达和先验强度，但通信事件总数受细胞群体大小影响。如果处理组中某个细胞群体（如成纤维细胞）比例升高，该群体参与的通信事件数量自然增加，总通信强度升高可能仅反映群体比例变化而非通讯活跃度提升。未归一化的比较会将群体大小差异误判为通讯强度差异。正确方法是在 `computeCommProb` 中设置 `population.size = TRUE`，CellChat 会将通信概率按细胞群体大小归一化，消除群体比例的影响。比较两个条件时应先确保使用相同的细胞类型注释，再用 `mergeCellChat` 合并对象，通过 `compareInteractions` 比较每个细胞群体的出向和入向通信强度差异。差异显著的通信应配合配体受体表达量验证，排除因细胞比例变化造成的假阳性。
:::

### 第3题 概念理解

某 PPI 网络的度分布服从幂律分布（无标度网络）。研究者随机移除 10% 的节点后网络保持连通，但移除度最高的 10% 节点后网络碎裂为多个小分量。解释两种移除策略产生不同结果的原因，并说明其生物学意义。

::: details 参考答案
无标度网络的度分布呈幂律衰减，少数 hub 节点拥有极高连接度，大多数节点连接度很低。随机移除节点大概率命中低连接度节点，这些节点是网络的末端，移除后对整体连通性影响很小，网络通过 hub 节点维持连通。靶向移除 hub 节点会切断大量连接路径，hub 节点承担的网络桥梁功能丢失，剩余节点失去相互连接的中间枢纽，网络快速碎裂。生物学意义在于：生物网络对随机突变（影响随机基因）具有鲁棒性，大多数基因突变不影响整体功能；但对 hub 基因的突变极度敏感，hub 基因突变往往致死或导致严重疾病。这解释了为何必需基因通常是网络中的 hub 节点，也解释了为何靶向 hub 蛋白的药物（如激酶抑制剂）往往有显著效果但副作用也大。网络鲁棒性分析可用于识别药物靶点和必需基因。
:::

## 常见错误

**错误 1 · WGCNA 模块不稳定，模块-性状关联微弱且模块划分在不同样本子集间差异大**

原因：软阈值幂（power）选择不当。power 过低（如 1-3）时网络过于密集，弱相关未被抑制，模块边界模糊；power 过高（如 15-20）时网络过于稀疏，真实共表达关系被切断。部分研究者跳过 `pickSoftThreshold` 直接使用默认值或随意选择 power，未检验网络是否满足无标度拓扑。样本量不足（低于 15 个）也会导致相关矩阵估计不稳定，模块划分随机性增大。

解决：用 `pickSoftThreshold` 扫描 power 1-20，选择 scale-free topology fit R² 首次超过 0.8 的 power 值，同时确认 mean connectivity 已降至平台期。若 R² 在所有 power 下都不达标，先检查数据质量：过滤低表达基因（如 `rowSums(counts) > 10`）和低变异基因（如 MAD 排序后取前 25%），必要时增加样本量。模块划分的稳定性可用 bootstrap 重采样评估，稳定模块在 70% 以上的重采样中应保持一致。WGCNA 至少需要 15 个样本，推荐 20 个以上。

**错误 2 · STRING 导入的 PPI 网络边数过多导致可视化杂乱、模块检测失效，或边数过少导致网络碎片化**

原因：STRING 置信度阈值（combined score）设置不当。STRING 的 combined score 整合实验验证、数据库注释、共表达、文本挖掘等多种证据，score 范围 0-1000。阈值过低（如 150）包含大量文本挖掘的低置信互作，网络中噪声边淹没真实信号；阈值过高（如 950）仅保留实验验证的高置信互作，遗漏物种特异或新发现的互作，导致网络断裂成孤立小分量。

解决：根据研究目标选择阈值。常规分析使用 score ≥ 700（high confidence），平衡覆盖度和可靠性。探索性分析可用 score ≥ 400（medium confidence）但需配合功能验证。严格因果推断使用 score ≥ 900。导入 STRING 时用 `score_threshold` 参数过滤，STRINGdb R 包中 `STRINGdb$new(score_threshold = 700)`。网络过密时可进一步用最大团（maximal clique）或 k-core 分解提取核心子网络。网络过疏时检查基因符号是否正确映射到 STRING ID，物种是否匹配。

**错误 3 · GSEA 结果中所有通路的 NES 接近 0，无显著富集通路，与 ORA 结果矛盾**

原因：基因排序方式错误。GSEA 要求输入按有符号统计量降序排列的基因列表，排在顶部的是上调基因，底部是下调基因。研究者误用 p 值排序（p 值小的基因在顶部，混合了上调和下调基因），或用绝对 logFC 排序（丢失方向信息），导致基因集内基因分散在排序两端，富集分数相互抵消。另一种错误是基因列表中包含 NA 或重复基因名，fgsea 会报错或给出错误结果。

解决：基因列表用有符号统计量排序，推荐使用收缩后的 log2FoldChange（apeglm 或 ashr 收缩）或 Wald 检验的 t 统计量。排序前用 `sort(gene_list, decreasing = TRUE)` 确保降序，剔除 NA 和 Inf 值，基因名为唯一。DESeq2 结果可用 `res$symbol` 配合 `res$log2FoldChange`（收缩后）构建排序列表，或直接用 `gseGO` / `gsePathway` 函数（内部自动处理排序）。GSEA 与 ORA 结果矛盾时优先参考 GSEA，因为它保留了全基因的方向信息，不受阈值切割影响。

**错误 4 · 通路富集分析报告中 p 值显著但 FDR 不显著，研究者仅报告 p 值导致大量假阳性通路**

原因：多重检验校正遗漏或不当。ORA 同时检验数十到数百个通路或 GO term，每个检验的 p 值未校正时假阳性率随检验次数线性增长。检验 100 个通路时，即使所有通路都无真实富集，仍期望约 5 个通路的 p 值小于 0.05。研究者仅报告 raw p 值或用 Bonferroni 校正（过于保守，导致假阴性）。

解决：所有富集分析必须报告 FDR 校正后的 q 值。Benjamini-Hochberg（BH）方法是标准选择，clusterProfiler 的 `enrichGO` 和 `enrichKEGG` 默认使用 BH 校正（`pAdjustMethod = "BH"`）。报告时用 `qvalueCutoff = 0.05` 筛选，同时报告 raw p 值和 FDR 供读者判断。GO 三层（BP、MF、CC）应分别校正，不混合。基因集数量极大时（如 MSigDB 全库）BH 校正可能过于保守，可考虑 qvalue 包的 Storey 方法估计 π0 提高检验功效。富集结果中 FDR < 0.05 的通路数量为零时，不应挑选 raw p 值最小的通路作为结论，应报告无显著富集。
