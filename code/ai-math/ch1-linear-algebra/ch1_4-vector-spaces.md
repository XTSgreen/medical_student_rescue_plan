...
1. **欧氏空间 $\mathbb{R}^n$**：常见的向量空间，元素是 $n$ 元实数组，加法和数乘按分量进行。
...
为什么只需这三条？因为其余公理（交换律、结合律等）在 $V$ 中已经成立，自然在子集 $W$ 中也成立——只要运算结果不**跑出** $W$。这是**封闭性**的核心意义。
...
张成是一个子空间——它包含零向量（所有 $c_i = 0$），且对加法和数乘封闭。$\text{span}(S)$ 是包含 $S$ 的**最小子空间**。
...
其中 $\mathbf{a}_j$ 是 $A$ 的第 $j$ 列。第二个等式表明，列空间就是**所有可能的输出 $A\mathbf{x}$**——这是矩阵所代表线性变换的**像空间**（Image / Range）。
...
这一结论的直觉是：列空间的基就是 $A$ 的**主元列**——线性无关的列向量。主元列的个数是 rank。因此列空间的维数等于秩。
...
行视角回答的问题是：**矩阵 $A$ 的行向量在输入空间中铺成了什么样的子空间？** 这一视角虽然不如列视角直观，但有着重要的几何意义。
...
在工程中，左零空间常对应系统的**守恒量**或**约束方程**。例如电路中的基尔霍夫电流定律、力学中的力平衡，都是某个矩阵的左零空间条件。
...
这一映射是**双射**（一一对应）——行空间的每个向量唯一对应列空间的一个向量，反之亦然。同构保持线性结构，因此行空间与列空间**代数上相同**——它们维数相等（都为 $r$），这是**行秩 = 列秩**的几何本质。
...
设 $V$ 是 $\mathbb{R}^n$ 的子空间，$V^\perp$ 是其正交补。证明 $V \cap V^\perp = \{\mathbf{0}\}$，并由此说明任意 $\mathbf{x} \in \mathbb{R}^n$ 的正交分解 $\mathbf{x} = \mathbf{x}_V + \mathbf{x}_{V^\perp}$ 是唯一的。

::: details 参考答案
先证交集只含零向量。设 $\mathbf{u} \in V \cap V^\perp$。由于 $\mathbf{u} \in V^\perp$，它与 $V$ 中所有向量正交；又由于 $\mathbf{u} \in V$，它必须与自身正交，即 $\mathbf{u} \cdot \mathbf{u} = 0$。由内积正定性 $\|\mathbf{u}\|^2 = 0$ 推出 $\mathbf{u} = \mathbf{0}$，故 $V \cap V^\perp = \{\mathbf{0}\}$。

再证分解唯一。设 $\mathbf{x}$ 有两种分解 $\mathbf{x} = \mathbf{x}_V + \mathbf{x}_{V^\perp} = \mathbf{x}'_V + \mathbf{x}'_{V^\perp}$。移项得 $\mathbf{x}_V - \mathbf{x}'_V = \mathbf{x}'_{V^\perp} - \mathbf{x}_{V^\perp}$。左侧属于 $V$（子空间对减法封闭），右侧属于 $V^\perp$（同理），故该向量同时落在 $V$ 与 $V^\perp$ 中，由前述结论只能为零向量。因此 $\mathbf{x}_V = \mathbf{x}'_V$，$\mathbf{x}_{V^\perp} = \mathbf{x}'_{V^\perp}$，分解唯一。这一唯一性是后续最小二乘解、PCA 投影等方法的数学基础。
:::

### 第 2 题 代码验证

利用本节的 `<FourSubspacesTheoremDemo>` 交互组件，切换到**秩 r=2** 预设。观察左侧 $\mathbb{R}^n$ 场景中绿色行空间（平面）与红色零空间（直线）的几何关系，记录两者的维数，验证 $\dim(C(A^T)) + \dim(N(A)) = n$；再观察右侧 $\mathbb{R}^m$ 场景中蓝色列空间与橙色左零空间，验证 $\dim(C(A)) + \dim(N(A^T)) = m$。

::: details 参考答案
$r=2$ 预设下，左侧输入空间 $\mathbb{R}^n$（$n=3$）中绿色行空间是二维平面，红色零空间是一维直线，两者**相互垂直**——零空间直线沿行空间平面的法向方向。维数验证：$\dim(C(A^T)) + \dim(N(A)) = 2 + 1 = 3 = n$，行空间与零空间正交且填满整个 $\mathbb{R}^n$。右侧输出空间 $\mathbb{R}^m$（$m=3$）中蓝色列空间是二维平面，橙色左零空间是一维直线，几何关系同样正交。维数验证：$\dim(C(A)) + \dim(N(A^T)) = 2 + 1 = 3 = m$，列空间与左零空间正交且填满整个 $\mathbb{R}^m$。两个场景共同印证四大子空间的正交互补结构。
:::

### 第 3 题 概念推导

设 $A = \begin{bmatrix} 1 & 2 & 3 \\ 2 & 4 & 6 \\ 1 & 1 & 1 \end{bmatrix}$。求 $A$ 的秩、四大子空间的维数，并指出每个子空间所在的欧氏空间（$\mathbb{R}^n$ 还是 $\mathbb{R}^m$）。

::: details 参考答案
$A$ 是 $3 \times 3$ 矩阵，$m = n = 3$。第二行是第一行的 $2$ 倍，故 $\text{rank}(A) = 2$（第一、三行线性无关）。

四大子空间维数与所在空间：

| 子空间 | 符号 | 所在空间 | 维数 |
|--------|------|----------|------|
| 列空间 | $C(A)$ | $\mathbb{R}^m = \mathbb{R}^3$ | $r = 2$ |
| 零空间 | $N(A)$ | $\mathbb{R}^n = \mathbb{R}^3$ | $n - r = 1$ |
| 行空间 | $C(A^T)$ | $\mathbb{R}^n = \mathbb{R}^3$ | $r = 2$ |
| 左零空间 | $N(A^T)$ | $\mathbb{R}^m = \mathbb{R}^3$ | $m - r = 1$ |

注意：虽然本例 $m = n$ 使四个子空间都落在 $\mathbb{R}^3$ 中，但它们的归属不同——零空间与行空间属于输入侧 $\mathbb{R}^n$，列空间与左零空间属于输出侧 $\mathbb{R}^m$。当 $m \neq n$ 时这一区别关键。正交补关系：行空间（二维平面）⊥ 零空间（一维直线）；列空间（二维平面）⊥ 左零空间（一维直线）。
:::

## 常见错误

**错误 1 · 子空间验证时遗漏不过原点这一条件**

原因：判断某集合是否为子空间时，只检查加法封闭与数乘封闭，忽视集合必须包含零向量这一要求。例如集合 $\{(x, y) : x + y = 1\}$ 是 $\mathbb{R}^2$ 中一条直线，加法与数乘看似成立，但不过原点（零向量 $(0,0)$ 不满足 $0+0=1$），该集合是仿射子空间，不满足子空间条件。

解决：子空间验证按三要素依次进行：**含零向量、加法封闭、数乘封闭**。其中含零向量等价于不过原点的集合直接排除。更简洁的做法是先检查 $\mathbf{0}$ 是否属于该集合，若不属于立即判定非子空间，无需继续验证封闭性。

**错误 2 · 把零空间放在 $\mathbb{R}^m$ 中讨论**

原因：混淆四大子空间所在的空间。零空间 $N(A)$ 由 $A\mathbf{x} = \mathbf{0}$ 定义，$\mathbf{x} \in \mathbb{R}^n$，故零空间是 $\mathbb{R}^n$ 的子空间。初学时容易因为**零空间与列空间都涉及 $A$**而把两者放在同一空间，导致正交补关系错配。

解决：按**输入侧/输出侧**分组记忆：输入侧 $\mathbb{R}^n$ 含行空间 $C(A^T)$ 与零空间 $N(A)$；输出侧 $\mathbb{R}^m$ 含列空间 $C(A)$ 与左零空间 $N(A^T)$。正交互补关系只发生在同侧之间：行空间 ⊥ 零空间（同在 $\mathbb{R}^n$），列空间 ⊥ 左零空间（同在 $\mathbb{R}^m$）。

**错误 3 · 把维数等同于基的向量长度**

原因：维数 $\dim(V)$ 是基中向量的**个数**，与每个向量的长度（分量数）无关。例如 $\mathbb{R}^3$ 中一条直线是一维子空间，基向量是 $(1, 2, 3)^T$，长度为 $3$，但维数是 $1$。初学时容易把向量长度当作维数。

解决：明确**维数 = 基的向量个数**这一规则。基向量本身的分量数由所在欧氏空间决定（$\mathbb{R}^n$ 中向量长度为 $n$），与子空间维数无直接关系。求维数时先找基，再数基向量的个数。

**错误 4 · 把正交补等同于集合补集**

原因：正交补 $V^\perp$ 是与 $V$ 中所有向量都正交的向量集合，不是 $V$ 在全集中的集合论补集 $\mathbb{R}^n \setminus V$。集合补集包含大量与 $V$ 不正交的向量，与正交补完全不同。

解决：明确**正交补**是几何概念，要求每个向量与 $V$ 中**所有**向量正交，而非简单地**不属于** $V$。验证某向量 $\mathbf{u}$ 是否在 $V^\perp$ 中，需要检查 $\mathbf{u}$ 与 $V$ 的一组基中每个向量都正交（点积为零），而不是检查 $\mathbf{u}$ 是否属于 $V$。
