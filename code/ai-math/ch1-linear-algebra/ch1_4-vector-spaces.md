...
前三节我们用"向量—矩阵—方程组"的语言解决了线性代数的基本计算问题。本节将完成一次重要的视角跃迁——从"具体的数表"上升到"抽象的空间"。我们将看到，矩阵 $A$ 不仅是一个数表，更是连接两个向量空间的桥梁：它把输入空间 $\mathbb{R}^n$ 映射到输出空间 $\mathbb{R}^m$，并在两个空间中各自刻画出两个子空间——**列空间、零空间、行空间、左零空间**。这"四大基本子空间"构成了线性代数最壮丽的结构图景，是理解最小二乘、SVD、PCA 等高级方法的核心。
...
不过原点的平面**不是**子空间（是**仿射集**，将在 1.4.7 节看到它与子空间的关系） ✗
...
张成的几何形状取决于向量的个数和是否共线：

- **单个非零向量** $\mathbf{v}$：张成一条过原点的直线（沿 $\mathbf{v}$ 方向）
- **两个不共线向量** $\mathbf{v}_1, \mathbf{v}_2$：张成一个过原点的平面
- **三个不共面向量** $\mathbf{v}_1, \mathbf{v}_2, \mathbf{v}_3$：张成整个 $\mathbb{R}^3$
- **两个共线向量**：张成仍是一条直线（不是平面，因为第二个向量没"贡献"新方向）
...
注意：是**原矩阵的列**，不是 RREF 的列！RREF 的主元列只是位置标记，真正的基向量来自原矩阵。
...
这一结论极为自然：$A\mathbf{x} = \mathbf{b}$ 有解 $\Leftrightarrow$ 存在 $\mathbf{x}$ 使 $A\mathbf{x} = \mathbf{b}$ $\Leftrightarrow$ $\mathbf{b}$ 是某些列的线性组合 $\Leftrightarrow$ $\mathbf{b} \in C(A)$。
...
零空间回答了另一个核心问题：**哪些输入被 $A$ "压缩"为零？** 这些向量在变换中"消失"了——它们携带的信息在变换后无法恢复。
...
行空间是 $\mathbb{R}^n$ 的子空间（与零空间处于同一空间！）。这一点至关重要——行空间和零空间都在输入空间 $\mathbb{R}^n$ 中，它们将共同分割 $\mathbb{R}^n$（见 1.4.7 节）。
...
这是线性代数最深刻的定理之一。它表明：**无论从行视角还是列视角看，矩阵承载的"独立信息量"是相同的**。行向量和列向量的"有效方向数"相等——尽管它们处于不同的空间（$\mathbb{R}^n$ vs $\mathbb{R}^m$）。
...
这一结构图景是后续章节的基础：1.5 节将用正交投影求解最小二乘问题，1.6 节将用特征值刻画矩阵的"本质方向"，1.7 节的 SVD 则是四大子空间的最美绽放——它把任意矩阵分解为"在行空间找正交基 + 在列空间找正交基 + 中间用奇异值连接"的完美形式。
