...
上一节我们建立了"四大子空间"的结构图景：行空间与零空间在 $\mathbb{R}^n$ 中互补，列空间与左零空间在 $\mathbb{R}^m$ 中互补。但"互补"只是维度上的加法关系——两个子空间的维数之和等于全空间维数。本节将进一步引入**正交**这一几何关系：互补的子空间不仅"填满"整个空间，而且"相互垂直"。这一性质让我们能把任意向量**唯一分解**为两个正交分量之和，从而引出**投影**这一核心算法。

投影是线性代数中最具工程价值的工具：当方程 $A\mathbf{x} = \mathbf{b}$ 无解时，我们寻找"最接近" $\mathbf{b}$ 的 $A\mathbf{x}$——这等价于把 $\mathbf{b}$ 投影到 $A$ 的列空间。**最小二乘法**正是这一思想的直接应用。而要让投影算法**数值稳定**，我们需要**标准正交基**与 **Gram-Schmidt 正交化**，最终凝聚为矩阵形式的 **QR 分解**。本节将沿着"正交 → 投影 → 最小二乘 → 标准正交基 → Gram-Schmidt → QR"这条主线，把正交性的几何直觉锻造为可计算的代数工具。
...
这一定理的工程意义非凡：**正交向量组天然线性无关**，无需再做无关性检验。这是标准正交基具有"绝佳数值性质"的根源。
...
回到 1.4 节的四大子空间，它们之间存在优美的**正交配对**（此处仅列举，不证明）：
...
这意味着在 $\mathbb{R}^n$ 中，行空间与零空间不仅互补（维数和为 $n$），而且**正交**（任一向量与其正交补中的向量都垂直）。同理，列空间与左零空间在 $\mathbb{R}^m$ 中正交互补。
...
第 2 条是投影的"灵魂"：投影点不是任意的 $S$ 中点，而是使得"差向量"垂直于整个子空间的特殊点。
...
::: tip 投影矩阵的"对偶性"
$P$ 和 $I - P$ 像一对孪生兄弟：$P$ 把向量拉到列空间，$I - P$ 把向量推到左零空间。两者共同作用将 $\mathbb{R}^m$ 完整分解为 $C(A) \oplus N(A^T)$。这种"对偶投影"在信号处理（信号 + 噪声分解）、统计学（解释 + 残差分解）中无处不在。
:::
...
**残差位于左零空间**。这一观察极其重要：它告诉我们最小二乘的"误差方向"是 $A$ 行向量的"约束方向"——即 $A^T \mathbf{e} = \mathbf{0}$，这正是法方程的来源。
...
法方程 $A^T A \hat{\mathbf{x}} = A^T \mathbf{b}$ 的数值病根在于：$A^T A$ 的**条件数是 $A$ 的平方**：
...
::: tip 标准正交基 = 数值"黄金坐标系"
把数据变换到标准正交基下，等价于把一个歪斜的坐标系"掰直"成笛卡尔坐标系。在新的坐标系下，所有计算（投影、最小二乘、条件数）都变得简单且数值稳定。这正是 PCA、SVD、QR 等方法在数据科学中"必经标准正交化"的根本原因。
:::
...
Gram-Schmidt 的几何直觉极其直观：**从新向量中减去它在已有正交方向上的"影子"**，剩下的部分就是与已有方向都垂直的新方向。
...
Gram-Schmidt 像一条加工流水线：原料是任意线性无关的向量组 $\{\mathbf{a}_k\}$，产物是标准正交组 $\{\mathbf{q}_k\}$，副产物是三角系数 $\{r_{ij}\}$。整条流水线的"操作"是简单的"减去投影 + 归一化"，无需解方程、无需矩阵求逆。这种简洁性使 Gram-Schmidt 成为最优雅的算法之一。
:::
...
法方程 $A^T A \hat{\mathbf{x}} = A^T \mathbf{b}$ 的病根：$\kappa(A^T A) = \kappa(A)^2$，条件数平方放大。
...
::: tip QR 分解 = "正交化 + 上三角化"
QR 分解把任意矩阵 $A$ 拆成"正交部分 $Q$"和"上三角部分 $R$"。这一拆解在数值线性代数中无处不在：解最小二乘、计算特征值（QR 算法）、正交化基、估计条件数等都依赖 QR。掌握 QR 分解的几何与代数双重意义，是进入高级数值方法（如 SVD、Schur 分解）的门槛。
:::
...
这对孪生投影矩阵共同作用，把空间"彻底分解"为两个正交部分。这一分解是信号处理、统计回归、机器学习的共同数学基础。
...
当 $A\mathbf{x} = \mathbf{b}$ 无解时，求 $\mathbf{p} \in C(A)$ 最近似 $\mathbf{b}$——这是从"精确求解"到"最佳逼近"的思维跃迁。这一跃迁是数据科学的起点：真实数据总有噪声，"完美拟合"既不可能也不必要，"最佳逼近"才是工程现实。
...
正交性不仅是线性代数的优雅理论，更是现代 AI 算法的"隐形骨架"：
...
::: note 正交性 = 线性代数的"中央车站"
本节的旅程"正交 → 投影 → 最小二乘 → 标准正交基 → Gram-Schmidt → QR"展示了正交性如何从抽象的几何概念逐步锻造为可计算的代数工具。每一站都对应一种工程应用：投影对应数据拟合，最小二乘对应回归分析，QR 对应数值稳定算法。下一节将进入特征值与特征向量——把"正交"扩展到"特征向量正交"（对称矩阵的可对角化性），最终通向 SVD 这一"线性代数的顶峰"。
:::
